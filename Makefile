...
build : $(PYON_TARGET) $(RTS_TARGET) \
	$(DATADIR)/include/pyon.h \
	$(DATADIR)/include/pyon_internal.h \
	$(DATADIR)/include/apply.h \
	$(DATADIR)/include/layout.h \
	$(DATADIR)/include/struct.h \
	$(DATADIR)/include/memory.h \
	$(DATADIR)/libpyonrts.so
...
# Copy header files to data directory
$(DATADIR)/include/pyon.h : $(SRCDIR)/rts/pyon.h
	mkdir -p $(DATADIR)/include
	cp $< $@

$(DATADIR)/include/pyon_internal.h : $(SRCDIR)/rts/pyon_internal.h
	mkdir -p $(DATADIR)/include
	cp $< $@

$(DATADIR)/include/struct.h : $(SRCDIR)/rts/struct.h
	mkdir -p $(DATADIR)/include
	cp $< $@

$(DATADIR)/include/apply.h : $(SRCDIR)/rts/apply.h
	mkdir -p $(DATADIR)/include
	cp $< $@

$(DATADIR)/include/layout.h : $(BUILDDIR)/rts/layout.h
	mkdir -p $(DATADIR)/include
	cp $< $@

$(DATADIR)/include/memory.h : $(SRCDIR)/rts/memory.h
	mkdir -p $(DATADIR)/include
	cp $< $@
