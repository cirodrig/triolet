                     ...
                     int previous_entry_is_triolet_library)
{
  ...
  int is_triolet_library;
  {
    char *filename = entry->filename;
    if (filename == NULL) {
      is_triolet_library = 0;
    } else {
      int slash_index = -1;
      int cur_index = 0;
      ...
      is_triolet_library =
        strncmp(filename + slash_index + 1,
                "libtrioletrts",
                strlen("libtrioletrts")) == 0;
    }
  }
  ...
  if (is_triolet_library || previous_entry_is_triolet_library) {
    ...
    if (Triolet_library_address_start == 0) {
      ...
      Triolet_library_address_start =
        Triolet_library_address_start < sa ? Triolet_library_address_start : sa;
      Triolet_library_address_end =
        Triolet_library_address_end > sa ? Triolet_library_address_end : ea;
    }
  }
}

/* Search for the Triolet libraries in the memory map */
void triolet_scan_memory_map(void)
{
  FILE *maps;
...
uint32_t
triolet_global_pointer_offset(void *p)
{
