
#include <stdio.h>
#include <stdlib.h> 
#include <string.h>
#include "CxxWrapperGeneratorTypes.h"
#include "CxxWrapperGeneratorPrint.h"
#include "Signature.h"
#include "CxxWrapperGenerator.h"

unsigned int variableNameCounter = 0;

/* Return 'True' if values of this type are passed to and returned from Pyon
 * code.  Return 'False' if values of this type are not passed.
 */
int
typePassedToPyon(const PyonType *t) {
  switch (t->tag) {
  case PyonNoneTypeTag: return 0; /* 'NoneType' is not passed as a parameter */
  default: return 1;
  }
}

void 
printCxxFunction(FILE* fp, char* wrappedFunctionName, char* wrapperFunctionName, PyonSignature* pyonSignature) {

  Pool *p = Pool_create();

  // Generate Functions
  Function* wrappedFunction = createWrappedFunctionDeclaration(p, wrappedFunctionName, pyonSignature);
  Function* wrapperFunction = createWrapperFunctionDefinition(p, wrapperFunctionName, pyonSignature, wrappedFunction);

  // Print ASTs
  printFunction(fp, wrappedFunction);
  fputs("\n", fp);
  printFunction(fp, wrapperFunction);
  fputs("\n\n", fp);

  Pool_destroy(p);

}

Function* 
createWrappedFunctionDeclaration(Pool* p, char* wrappedFunctionName, PyonSignature* pyonSignature) {

  // Declare parameter data
  int parameterCount;
  Declaration** parameterList;

  {
    // There may be fewer than max_parameters, if some parameters are not passed
    int max_parameters = pyonSignature->parameterTypes->count;
    parameterList = malloc(max_parameters * sizeof(Declaration*));
    parameterCount =
      populateWrappedParameterList(p, parameterList, pyonSignature->parameterTypes);
  }

  // Create function signature
  Type* returnType = pyonToWrappedReturnType(p, pyonSignature->returnType);
  Type* signatureType =
    Type_function_create(p, returnType, parameterCount, parameterList);
  Declaration* signature =
    Declaration_extern_c_create(p, signatureType, Name_create_dynamic(p, strdup(wrappedFunctionName)));

  return Function_declaration_create(p, signature);

}

Function* 
createWrapperFunctionDefinition(Pool* p, char* wrapperFunctionName, PyonSignature* pyonSignature, Function* wrappedFunction) {
  
  // Extract return type
  Type* returnType =
    pyonToWrapperParamAndReturnType(p, pyonSignature->returnType);
  
  // Extract parameters
  const PyonTypes *pyonParameterTypes = pyonSignature->parameterTypes;
  int wrapperParameterCount = pyonParameterTypes->count;
  int wrappedParameterCount =
    wrappedFunction->signature->type->function.parameterCount;

  Declaration** wrappedParameterList =
    wrappedFunction->signature->type->function.parameterList;
  int parameterIndex;

  /* Create the wrapper function's parameters */
  Declaration **wrapperParameterList =
    createWrapperFunctionParameters(p, pyonSignature);
  
  // Create signature
  Type* signatureType =
    Type_function_create(p, returnType, wrapperParameterCount, wrapperParameterList);
  Declaration* signature =
    Declaration_local_create(p, signatureType, Name_create_dynamic(p, strdup(wrapperFunctionName)));

  // Create declaration list for the variables that will be passed to the wrapped function
  DeclarationList* declarationList = DeclarationList_create(p);
  {
    Declaration **p_current_wrapped_param = wrappedParameterList;
    const PyonType **p_current_type = pyonParameterTypes->elems;

    // Reuse wrapped parameter list since the types will be the same
    for (parameterIndex=0;
         parameterIndex<wrapperParameterCount;
         parameterIndex++) {
      if (typePassedToPyon(*p_current_type)) {
        DeclarationList_append(p, declarationList, *p_current_wrapped_param);
        p_current_wrapped_param++;
      }
      p_current_type++;
    }
  }

  int has_return_variable;
  /* If has_return_variable, these are the name and type of the local
   * variable holding the return value */
  Name* returnVariableName;
  Type* returnVariableType;

  // Determine the wrapped variable's return type, if any
  returnVariableType =
    pyonToWrapperReturnDeclarationType(p, pyonSignature->returnType);
  has_return_variable = returnVariableType != NULL;
  returnVariableName = has_return_variable ? newVariableName(p) : NULL;

  if (has_return_variable)
    DeclarationList_append(p, declarationList,
                           Declaration_local_create(p, returnVariableType, returnVariableName));
  
  // Create statement list
  StatementList* statementList = StatementList_create(p);
  // Step 1: unwrap parameters
  {
    Declaration **p_current_wrapped_param = wrappedParameterList;
    Declaration **p_current_wrapper_param = wrapperParameterList;
    const PyonType **p_current_type = pyonParameterTypes->elems;

    for (parameterIndex=0; parameterIndex<wrapperParameterCount; parameterIndex++) {
      if (typePassedToPyon(*p_current_type)) {
        Statement* unwrappingStatement = 
          createUnwrappingStatement(p, *p_current_type,
                                    *p_current_wrapped_param,
                                    *p_current_wrapper_param);
        StatementList_append(p, statementList, unwrappingStatement);
        p_current_wrapped_param++;
        p_current_wrapper_param++;
      }
      else {
        p_current_wrapper_param++;
        // There is no wrapped param for this parameter
      }
      p_current_type++;
    }
  }

  // Step 2: invoke wrapped function
  // Step 2.2: generate function invokation for the right hand side
  TmplName* functionTmplName =
    TmplName_create_zero_types(p, wrappedFunction->signature->name);
  QName *functionQName =
    QName_tmplName_create(p, functionTmplName);
  Expression** argumentList =
    malloc(wrappedParameterCount*sizeof(Expression*));
  populateArgumentList(p, argumentList, wrappedParameterList, wrappedParameterCount);
  Expression* functionQNameExpression =
    Expression_objectID_create(p, functionQName);
  Expression* functionInvocationExpression =
    Expression_function_call_create(p, functionQNameExpression, wrappedParameterCount, argumentList);

  // Generate a statement that calls the function.
  // Assign the return value to a variable.
  Statement* statement;
  Expression* returnVariableExpression;
  if (has_return_variable) {
    returnVariableExpression =
      nameToObjectIDExpression(p, returnVariableName);
    statement = Statement_assignment_create(p, returnVariableExpression, functionInvocationExpression);
  }
  else {
    returnVariableExpression = NULL;
    statement = Statement_expression_create(p, functionInvocationExpression);
  }
  StatementList_append(p, statementList, statement);

  // Step 3: wrap final value and return
  switch (pyonTypeKind(pyonSignature->returnType)) {
    case ValTypeTag:
    case BoxTypeTag:
    {
      // Construct the return value from the data type
      QName* constructorQName = returnType->qName;
      Expression** constructorArgumentList;
      int argumentCount;
      if (has_return_variable) {
        argumentCount = 1;
        constructorArgumentList = malloc(1*sizeof(Expression*));
        constructorArgumentList[0] = returnVariableExpression;
      }
      else {
        argumentCount = 0;
        constructorArgumentList = NULL;
      }

      // Create the return expression
      Expression* returnExpression =
        Expression_constructor_create(p, constructorQName, argumentCount, constructorArgumentList);
      statement = Statement_return_create(p, returnExpression);
      StatementList_append(p, statementList, statement);
      break;
    }
    case BareTypeTag: {
      if (!has_return_variable) {
        ERR("Unexpected return type in createWrapperFunctionDefinition");
      }

      // Construct the return value from the data type:
      // > return unboxBareObject<ReturnType>(x);
      Type** type_list = malloc(sizeof(Type *));
      type_list[0] = returnType;
      QName *function_name =
        QName_namespace_qName_create(p, Name_create_static(p, "Triolet"),
                                     QName_tmplName_create(p, TmplName_create(p, Name_create_static(p, "unboxBareObject"), 1, type_list)));

      Expression **argument_list = malloc(sizeof(Expression *));
      argument_list[0] = returnVariableExpression;
      Expression *function_call =
        Expression_function_call_create(p, Expression_objectID_create(p, function_name), 1, argument_list);

      // Create return type
      statement = Statement_return_create(p, function_call);
      StatementList_append(p, statementList, statement);
      break;
    }
    default: ERR("invalid PyonTypeTag in function createWrapperFunctionDefinition(Pool*, char*, PyonSignature*, Function*)");
  }

  return Function_definition_create(p, signature, declarationList, statementList);
  
}

// Create the parameter list for a wrapper function.  The parameter list has 
// one variable declaration for each parameter in the function signature.
// Returns a newly allocated array of length pyonSignature->parameterTypes->count.
Declaration**
createWrapperFunctionParameters(Pool *p, PyonSignature *pyonSignature)
{
  const PyonTypes *parameterTypes = pyonSignature->parameterTypes;
  int count = parameterTypes->count;
  Declaration** parameters = malloc(count * sizeof(Declaration*));

  int i;

  /* Create the wrapper function's parameters */
  for (i = 0; i < count; i++) {
    Type* t = pyonToWrapperParamAndReturnType(p, parameterTypes->elems[i]);
    parameters[i] = Declaration_local_create(p, t, newVariableName(p));
  }

  return parameters;
}


Statement*
createUnwrappingStatement(Pool* p, const PyonType* pyonType, Declaration* wrappedVariable, Declaration* wrapperVariable) {

  // Left hand side (lhs)
  Expression* lhsExpression = nameToObjectIDExpression(p, wrappedVariable->name);

  // Right hand side (rhs)
  TmplName* rhsVariableTmplName = TmplName_create_zero_types(p, wrapperVariable->name);
  QName* rhsVariableQName = QName_tmplName_create(p, rhsVariableTmplName);
  Expression* rhsExpression;
  switch (pyonTypeKind(pyonType)) {
    case ValTypeTag:
    case BoxTypeTag:
      rhsExpression = Expression_cast_create(p, wrappedVariable->type, rhsVariableQName);
      break;
    case BareTypeTag: {
      Expression* objectID = Expression_objectID_create(p, rhsVariableQName);
      Expression* memberAccessExpression = Expression_member_access_create(p, objectID, Name_create_static(p, "getBareData"));
      rhsExpression = Expression_function_call_create(p, memberAccessExpression, 0, NULL);
      break;
    }
    default: ERR("invalid PyonTypeTag in function createUnwrappingStatement(Pool*,PyonType*,Declaration*,Declaration*)");
  }

  // lhs = rhs
  return Statement_assignment_create(p, lhsExpression, rhsExpression);

}

Type* 
pyonToWrappedValType(Pool* p, const PyonType* pyonType) {
  switch (pyonType->tag) {
    case PyonIntTag:
      return Type_primType_create(p, INT32_T);
      break;
    case PyonFloatTag:
      return Type_primType_create(p, FLOAT);
      break;
    case PyonBoolTag:
      return Type_primType_create(p, BOOL_TYPE);
      break;
    case PyonNoneTypeTag:
      /* Error, NoneType is not passed to the wrapped function */
    default: ERR("invalid PyonTypeTag in function pyonToWrappedValType(Pool*,PyonType*)");
  }
}

// Convert the parameter type to a wrapped parameter type.
Type* pyonToWrappedParamType(Pool* p, const PyonType* pyonType) {
  switch(pyonTypeKind(pyonType)) {
    case ValTypeTag:
      return pyonToWrappedValType(p, pyonType);
    case BareTypeTag:
      return createTmplNameInNamespace (p, TmplName_create_zero_types(p, Name_create_static(p, "TriBarePtr")));
    default: ERR("invalid TypeKindTag in function pyonToWrappedParamType(Pool*, PyonType*)");
  }
}

// Get the return type of a function corresponding to the given Pyon return
// type
Type* pyonToWrappedReturnType(Pool* p, const PyonType* pyonType) {
  switch(pyonTypeKind(pyonType)) {
    case ValTypeTag:
    case BoxTypeTag:
      if (typePassedToPyon(pyonType))
        return pyonToWrappedValType(p, pyonType);
      else
        return Type_primType_create(p, VOID);
    case BareTypeTag:
      // Return a boxed object pointer
      return createTmplNameInNamespace (p, TmplName_create_zero_types(p, Name_create_static(p, "TriBoxPtr")));
    default: ERR("invalid TypeKindTag in function pyonToWrappedReturnType(Pool*, PyonType*)");
  }
}

Type* pyonToWrapperParamAndReturnType(Pool* p, const PyonType* pyonType) {
  return pyonToWrapperType(p, pyonType);
}

// Get the type of the local variable holding the return value.
// Return NULL if there's no local variable holding the return value.
Type* pyonToWrapperReturnDeclarationType(Pool* p, const PyonType* pyonType) {
  switch (pyonTypeKind(pyonType)) {
    case ValTypeTag:
    case BoxTypeTag:
      if (typePassedToPyon(pyonType))
        return pyonToWrappedValType(p, pyonType);
      else
        return NULL;

    case BareTypeTag: {
      // Returns a box object containing the bare object
      return createTmplNameInNamespace (p, TmplName_create_zero_types(p, Name_create_static(p, "TriBoxPtr")));
#if 0
      Type** typeList = malloc(sizeof(Type*));
      typeList[0] = pyonToWrapperType(p, pyonType);
      TmplName* tmplName = TmplName_create(p, Name_create_static(p, "Incomplete"), 1, typeList);
      return createTmplNameInNamespace (p, tmplName);
      break;
#endif
    }
    default: ERR("invalid TypeKindTag in function pyonToWrapperReturnType(Pool*, PyonType*)");
  }
}

Type* 
pyonToWrapperType(Pool* p, const PyonType* pyonType) {
  switch (pyonType->tag) {
    case PyonIntTag:
      return createTmplNameInNamespace (p, TmplName_create_zero_types(p, Name_create_static(p, "Int")));
      break;
    case PyonFloatTag:
      return createTmplNameInNamespace (p, TmplName_create_zero_types(p, Name_create_static(p, "Float")));
      break;
    case PyonBoolTag:
      return createTmplNameInNamespace (p, TmplName_create_zero_types(p, Name_create_static(p, "Bool")));
      break;
    case PyonNoneTypeTag:
      return createTmplNameInNamespace (p, TmplName_create_zero_types(p, Name_create_static(p, "NoneType")));
      break;
    case PyonTupleTypeTag: {
      int templateCount = pyonType->tuple.count;
      Type** typeList = malloc(templateCount*sizeof(Type*));
      int typeIndex;
      for (typeIndex=0; typeIndex<templateCount; typeIndex++) {
        typeList[typeIndex] = pyonToWrapperType(p, pyonType->tuple.elems[typeIndex]);
      }
      TmplName* tmplName = TmplName_create(p, Name_create_static(p, "Tuple"), templateCount, typeList);
      return createTmplNameInNamespace (p, tmplName);
      break;
    }
    case PyonListTypeTag: {
      // Create either a "List" or "BList" type
      const char *list_name = pyonType->list.boxed ? "BList" : "List";
      Type** typeList = malloc(sizeof(Type*));
      typeList[0] = pyonToWrapperType(p, pyonType->list.elem);
      TmplName* tmplName =
        TmplName_create(p, Name_create_static(p, list_name), 1, typeList);
      return createTmplNameInNamespace (p, tmplName);
      break;
    }
    case PyonArrayTypeTag: {
      Type** typeList = malloc(sizeof(Type*));
      typeList[0] = pyonToWrapperType(p, pyonType->array.elem);
      // Allocate string
      // 6 ("BArray") + 1 (dimension digit) + 1 (null terminator) = 8
      char *nameString = malloc(8*sizeof(char));
      if (pyonType->array.boxed) {
        sprintf(nameString, "BArray%d", pyonType->array.dimensionality);
      }
      else {
        sprintf(nameString, "Array%d", pyonType->array.dimensionality);
      }
      TmplName* tmplName = TmplName_create(p, Name_create_dynamic(p, nameString), 1, typeList);
      return createTmplNameInNamespace (p, tmplName);
      break;
    }
    default: ERR("invalid PyonTypeTag in function pyonToWrapperType(Pool*,PyonType*)");
  }
}

TypeKindTag pyonTypeKind(const PyonType *pyonType) {
  switch (pyonType->tag) {
    case PyonIntTag: case PyonFloatTag: case PyonBoolTag: case PyonNoneTypeTag:
      return ValTypeTag;
      break;
    case PyonTupleTypeTag: case PyonListTypeTag: case PyonArrayTypeTag:
      return BareTypeTag;
      break;
    default: ERR("invalid PyonTypeTag in function pyonTypeKind(PyonType*)");
  }
}

// For each parameter type, add a declaration to the parameter list
// if a parameter will be passed to the wrapped function.
// A declaration is added for most types.
// However, 'NoneType' is not passed as a parameter.
int
populateWrappedParameterList(Pool* p,
                             Declaration** parameterList,
                             const PyonTypes* pyonParameterTypes) {
  int num_wrapped_parameters = 0;
  int parameterCount = pyonParameterTypes->count;
  int parameterIndex;
  for (parameterIndex=0; parameterIndex<parameterCount; parameterIndex++){
    const PyonType *pyon_type = pyonParameterTypes->elems[parameterIndex];
    if (typePassedToPyon(pyon_type)) {
      Type* parameterType = pyonToWrappedParamType(p, pyon_type);
      if (parameterType) {
        // Append a parameter declaration to the array
        parameterList[num_wrapped_parameters++] =
          Declaration_local_create(p, parameterType, newVariableName(p));
      }
    }
  }
  return num_wrapped_parameters;
}

void 
populateArgumentList(Pool* p,
                     Expression** argumentList,
                     Declaration** wrappedParameterList,
                     int parameterCount) {
  int parameterIndex;
  for (parameterIndex=0; parameterIndex<parameterCount; parameterIndex++){
    argumentList[parameterIndex] = nameToObjectIDExpression(p, wrappedParameterList[parameterIndex]->name);;
  }
}

Name*
newVariableName(Pool* p) {
  unsigned int n = variableNameCounter++;
  unsigned int digits = 1;
  {
    unsigned int limit = 10;
    while (n >= limit) limit *= 10, digits++;
  }
  char *s = malloc(digits + 3);
  sprintf(s, "x_%u", n);
  return Name_create_dynamic(p, s);
}

Type*
createTmplNameInNamespace (Pool* p, TmplName* tmplName) {
      QName* qName = QName_tmplName_create(p, tmplName);
      qName = QName_namespace_qName_create(p, Name_create_static(p, "Triolet"), qName);
      return Type_qName_create(p, qName);
}

Expression* 
nameToObjectIDExpression(Pool* p, Name* name) {
    TmplName* tmplName = TmplName_create_zero_types(p, name);
    QName* objectID = QName_tmplName_create(p, tmplName);
    Expression* expression = Expression_objectID_create(p, objectID);
    return expression;
}




/*******************************************************************************
 **                       Example Wrapper Function                            **
 *******************************************************************************

      // WRAPPED FUNCTION
      bool sampleFunction(int_32 i,float f, PyonBarePtr tuple);

      // WRAPPER FUNCTION
      Bool sampleFunction_wrapper(Int i_wrapper, Float f_wrapper, Tuple<T1,T2> t) {

        // DECLARATIONS
        int_32 i; // Function argument
        float f; // Function argument
        PyonBarePtr t; // Function argument
        bool b; // Function return value
      
        // STATEMENTS
      
        // Step 1: unwrap parameters
        i = (int_32) i_wrapper;
        f = (float) f_wrapper;
        t = t.getBareData();

        // Step 2: invoke wrapped function
        b = sampleFunction(i,f,t);

        // Step 3: wrap final value and return
        return Bool(b);

      }
  
 ******************************************************************************/

