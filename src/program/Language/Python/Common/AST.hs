...
data Statement annot 
   ...
   -- | Export statement.
   | Export
     { export_lang :: Ident annot    -- ^ Language to export to.
     , export_name :: Maybe String   -- ^ Exported function name.
     , export_item :: Ident annot   -- ^ Local function name.
     , export_type :: Expr annot     -- ^ Exported function type.
     , stmt_annot :: annot
     }
   -- | While loop. 
   | While 
   ...
   -- | Require statement. 
   | Require
     { require_name :: Ident annot -- ^ Variable being required
     , require_label :: Expr annot -- ^ Requirement label.
     , stmt_annot :: annot
     }
...
data Argument annot
   ...
   = ArgExpr { arg_expr :: Expr annot, arg_py_annotation :: Maybe (Expr annot), arg_annot :: annot }
...
-- | Comprehension local definition. 
data CompLet annot =
   CompLet
   { comp_let_target :: Expr annot
   , comp_let_expr :: Expr annot
   , comp_let_iter :: Maybe (CompIter annot)
   , comp_let_annot :: annot 
   }
   deriving (Eq,Ord,Show,Typeable,Data)

type CompLetSpan = CompLet SrcSpan

instance Span CompLetSpan where
   getSpan = annot 

instance Annotated CompLet where
   annot = comp_let_annot 
...
data CompIter annot 
   ...
   | IterLet { comp_iter_let :: CompLet annot, comp_iter_annot :: annot }
   deriving (Eq,Ord,Show,Typeable,Data)
...
data Expr annot
   ...
   -- | Let expression (an extension to Python).
   | LetExpr { let_target :: [Expr annot], let_rhs :: Expr annot, let_body :: Expr annot, expr_annot :: annot }
