...
makeRBinOp :: ExprSpan -> [(OpSpan, ExprSpan)] -> ExprSpan
makeRBinOp e es
   = let -- Reverse the list, and associate each operator with the 
         -- preceding expression
         (ops, es') = unzip es
         (re:res)   = reverse (e:es')
         res'       = zip (reverse ops) res
     in foldl' mkOp re res'
   where
   -- Notice that order of operands is reversed
   mkOp e2 (op, e1) = BinaryOp op e1 e2 (spanning e1 e2)
