...
instance Pretty (Statement a) where
   ...
   pretty stmt@(Export {}) = text "export" <+> pretty (export_lang stmt) <+> maybe empty pretty (export_name stmt) <+> pretty (export_item stmt) <+> text ":" <+> pretty (export_type stmt)
   pretty stmt@(While {})
   ...
   pretty (Require { require_name = i, require_label = e }) = text "require" <+> pretty i <+> colon <+> pretty e
...
instance Pretty (Argument a) where
   pretty (ArgExpr { arg_expr = e, arg_py_annotation = Nothing }) = pretty e
   pretty (ArgExpr { arg_expr = e, arg_py_annotation = Just e' }) = pretty e <+> colon <+> pretty e' 
...
instance Pretty (CompLet a) where
   pretty (CompLet {comp_let_target = e1, comp_let_expr = e2})
      = text "let" <+> pretty e1 <+> equals <+> pretty e2
...
instance Pretty (CompIter a) where
   ...
   pretty (IterLet { comp_iter_let = compLet }) = pretty compLet
...
instance Pretty (Expr a) where
   ...
   pretty (LetExpr { let_target = target, let_rhs = rhs, let_body = body })
      = text "let" <+> commaList target <+> text "=" <+> pretty rhs <+> text "in" <+> pretty body
