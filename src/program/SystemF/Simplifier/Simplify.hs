...
rwCase2 is_stream_arg inf alts scrut' scrut_val = do
  ...
  case codeValue scrut_val of
    ...
    BoolAV (AbsValueProp var value)
      | Just trivial_value <- codeTrivialExp value -> do
          -- This is a boolean case statement.
          -- Cannot eliminate the case statement, but we can replace
          -- 'var' by 'trivial_value' in one branch of the case statment.
          let v_subst = singletonV var (SubstitutedVar trivial_value)
              subst = Subst Substitute.empty v_subst
          alts' <- map_true_branch (substitute subst) alts
  ...
  where
    ...
    -- Apply a function to the true branch of the case statement
    map_true_branch :: (AltSM -> LR AltSM) -> [AltSM] -> LR [AltSM]
    map_true_branch f alts = mapM (apply_to_alt_if is_true f) alts
      where
        is_true (AltSM alt) =
          case altCon alt
          of VarDeCon v [] [] -> v `isPyonBuiltin` The_True
             _ -> False

    apply_to_alt_if predicate f alt
      | predicate alt = f alt
