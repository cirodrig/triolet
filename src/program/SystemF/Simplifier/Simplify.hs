...
builtinFunctionSimplifiers =
  ...
  where
    entries =
      ...
      , (coreBuiltin The_toCursor, 1, 1, rwToCursorApp)
      , (coreBuiltin The_fromCursor, 1, 1, rwFromCursorApp)
      ]
...
rwToCursorApp inf [ty] [arg] = do
  (arg', arg_val) <- rwExp arg

  -- Special abstract value for the cursor
  let val = valueCode $ CursorAV arg_val
  return (varAppE inf (coreBuiltin The_toCursor) [ty] [arg'], val)

rwFromCursorApp inf [ty] [arg] = do
  (arg', arg_val) <- rwExp arg

  -- If argument is a cursor created from a known value, get the known value
  case codeValue arg_val of
    CursorAV val -> do
      m_val' <- liftTypeEvalM $ concretize ty val
      case m_val' of
        Just e  -> return (e, val)
        Nothing -> rebuild arg'
    _ -> rebuild arg'
  where
    rebuild arg' =
      return (varAppE inf (coreBuiltin The_fromCursor) [ty] [arg'], topCode)
