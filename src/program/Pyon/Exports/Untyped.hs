...
import qualified Pyon.LowLevel.BuiltinCalls as LowLevel
...
foreign export ccall pyon_makeBuiltinPrimOps :: PyPtr -> PyPtr -> IO PyPtr

pyon_makeBuiltinPrimOps _self mod = rethrowExceptionsInPython $ do
  expectHsObject mod
  m <- fromHsObject' mod
  newHsObject =<< LowLevel.makeBuiltinPrimOps m
...
foreign export ccall pyon_closureConvert :: PyPtr -> PyPtr -> IO PyPtr

pyon_closureConvert _self mod = rethrowExceptionsInPython $ do
  expectHsObject mod
  m <- fromHsObject' mod
  newHsObject =<< LowLevel.closureConvert m

foreign export ccall pyon_insertReferenceCounting :: PyPtr -> PyPtr -> IO PyPtr

pyon_insertReferenceCounting _self mod = rethrowExceptionsInPython $ do
  expectHsObject mod
  m <- fromHsObject' mod
